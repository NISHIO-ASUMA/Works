...
// キーボードクラスのコンストラクタ
//=========================================================
CInputKeyboard::CInputKeyboard() : m_nKeyPressCount(NULL)
{
	// 値のクリア
...
// マウスのコンストラクタ
//=========================================================
CInputMouse::CInputMouse() : m_CurrentMouseState{},
m_PrevState{}
{
	// 値のクリア
	m_pDevice = NULL;
...
// マウス移動量の取得
//=========================================================
D3DXVECTOR2 CInputMouse::GetMov(void)
{
	// 移動量変数
	D3DXVECTOR2 move;

	// 現在と過去フレームの差分を計算
	move.x = static_cast<float>(m_CurrentMouseState.lX - m_PrevState.lX);
	move.y = static_cast<float>(m_CurrentMouseState.lY - m_PrevState.lY);
